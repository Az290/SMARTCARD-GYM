...
public static final int AVATAR_STANDARD_SIZE = 128; // 48x48 pixels
...
*/
private byte[] packField(String value, byte fieldType) {
        if (value == null) return new byte[0];
        byte[] valBytes = value.getBytes(java.nio.charset.StandardCharsets.UTF_8);
        
        // Format: [Type][LenH][LenL][Value...]
        byte[] data = new byte[3 + valBytes.length];
        data[0] = fieldType;
        data[1] = (byte)((valBytes.length >> 8) & 0xFF);
        data[2] = (byte)(valBytes.length & 0xFF);
        System.arraycopy(valBytes, 0, data, 3, valBytes.length);
        
        return data;
    }

    public boolean updateInfoFields(String name, String phone, String email, String birthDate, String address) {
        if (!pinVerified || !cardRsaKeyLoaded) return false;

        try {
            System.out.println("\n[INFO] 📤 UPDATE INFO: Đang đóng gói dữ liệu...");

            // 1. Handshake (Gửi Session Key) - GIỮ NGUYÊN
            generateSessionKey();
            ...
            if (sendAPDU(apduKey).getSW() != 0x9000) return false;

            // 2. Đóng gói các trường (Chưa mã hóa)
            ByteArrayOutputStream rawData = new ByteArrayOutputStream();
            int fieldCount = 0;

            if (name != null && !name.isEmpty()) { rawData.write(packField(name, (byte)0x01)); fieldCount++; }
            if (phone != null && !phone.isEmpty()) { rawData.write(packField(phone, (byte)0x02)); fieldCount++; }
            if (email != null && !email.isEmpty()) { rawData.write(packField(email, (byte)0x03)); fieldCount++; }
            if (birthDate != null && !birthDate.isEmpty()) { rawData.write(packField(birthDate, (byte)0x04)); fieldCount++; }
            if (address != null && !address.isEmpty()) { rawData.write(packField(address, (byte)0x05)); fieldCount++; }

            // Thêm byte số lượng trường vào đầu
            ByteArrayOutputStream finalPayload = new ByteArrayOutputStream();
            finalPayload.write((byte) fieldCount);
            finalPayload.write(rawData.toByteArray());
            
            byte[] plainPayload = finalPayload.toByteArray();

            // 3. Encrypt lớp vận chuyển (Session Key) để bảo vệ trên đường truyền
            byte[] encryptedPayload = encryptWithSessionKey(plainPayload);

            // 4. Gửi xuống thẻ
            CommandAPDU apduData = new CommandAPDU(CLA, INS_UPDATE_INFO_ENCAPS, 0x00, 0x00, encryptedPayload);
            return sendAPDU(apduData).getSW() == 0x9000;
...
public byte[] signTransaction(String transactionData) {
        if (!pinVerified) {
            System.out.println("[Sign] ❌ PIN not verified, cannot sign!");
            return null;
        }

        try {
            System.out.println("[Sign] ✍️ Signing data: " + transactionData);
            byte[] data = transactionData.getBytes(java.nio.charset.StandardCharsets.UTF_8);

     
            CommandAPDU apdu = new CommandAPDU(CLA, INS_RSA_SIGN, 0x00, 0x00, data);
            ResponseAPDU resp = sendAPDU(apdu);

            if (resp != null && resp.getSW() == 0x9000) {
                byte[] signature = resp.getData();
                System.out.println("[Sign] ✅ Signature created: " + signature.length + " bytes");
                return signature;
            } else {
                System.out.println("[Sign] ❌ Signing failed: SW=" + String.format("%04X", resp != null ? resp.getSW() : 0));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
...
public void setRecoveryPhone(String phone) {
    ...
    // MỚI: Gửi SĐT xuống thẻ JavaCard để lưu làm recovery key
    if (phone != null && !phone.isEmpty()) {
        try {
            byte[] phoneBytes = phone.getBytes("UTF-8");
            
            // Gửi APDU để lưu recovery phone vào thẻ
            // Sử dụng INS_UPDATE_INFO với P1=0x01 để chỉ định đây là recovery phone
            CommandAPDU apdu = new CommandAPDU(CLA, (byte) 0x15, 0x00, 0x00, phoneBytes);
            ResponseAPDU resp = sendAPDU(apdu);
            
            if (resp != null && resp.getSW() == 0x9000) {
                System.out.println("[CARD] ✅ Recovery phone saved to card: " + phone);
            } else {
                System.out.println("[CARD] ⚠️ Failed to save recovery phone to card");
            }
        } catch (Exception e) {
            System.out.println("[CARD] ❌ Error saving recovery phone: " + e.getMessage());
        }
    }
}
